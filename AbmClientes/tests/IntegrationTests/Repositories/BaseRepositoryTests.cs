...
{
    public abstract class BaseRepositoryTests<T> : IDisposable where T : BaseEntity
    {
        protected readonly AbmClientesDbContext _context;

        protected IAsyncRepository<T> _asyncRepository;

        protected BaseRepositoryTests()
        {
            var options = new DbContextOptionsBuilder<AbmClientesDbContext>()
                .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
                .Options;
            
            _context = new AbmClientesDbContext(options);
